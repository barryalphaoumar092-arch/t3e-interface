...
// TEMPORAIRE - verifie le fix de positionnement bordereau (NOM/SPECIALITE/ADRESSE), a retirer apres usage
router.get('/_debug-test-bordereau-fill', async (req, res) => {
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="test-fill.docx"');
  res.send(docxBuf);
});
