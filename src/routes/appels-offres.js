...
router.get('/_debug-categories', async (req, res) => {
  const db = req.db;
  try {
    const r = await db.execute(`SELECT id, nom, (SELECT COUNT(*) FROM documents WHERE categorie_id = categories.id AND statut='actif') as nb FROM categories ORDER BY nom`);
    res.json(r.rows);
  } catch (e) {
